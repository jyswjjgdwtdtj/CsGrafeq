...
internal static class IntervalSetExtension
{
    ...
    {
        if (set.IsNumber) set = IntervalSet.Create(new Ranges(new Range(set.Number)), set.Def);
    }

    /// <summary>
    ///     单调增
    /// </summary>
    public static unsafe void MonotoneTransform(this Ranges ranges, delegate*<double, double> callback)
    {
    ...
    ///     单调减
    /// </summary>
    public static unsafe void MonotoneTransformOp(this Ranges ranges, delegate*<double, double> callback)
    {
        if (ranges.Length == 0)
            return;
    ...
    {
        foreach (ref var i in i1.Intervals) i = handler(i);
