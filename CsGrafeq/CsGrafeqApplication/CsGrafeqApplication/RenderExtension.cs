...
namespace CsGrafeqApplication
{
    internal class RenderExtension
    {
        public static void RenderMovedPlace(SKCanvas dc, RenderHandler rm, Size size, Point next, Point previous)
        {
            ...
            {
                rm.Invoke(dc, new(width - horizonDelta, 0, width, height));
                if (next.Y < previous.Y) //向上移动
                    rm.Invoke(dc,
                        new(0, height - verticalDelta, width - horizonDelta, height));
                else if (next.Y > previous.Y) //向下移动
                    rm.Invoke(dc, new(0, 0, width - horizonDelta, verticalDelta));
            }
            else if (next.X > previous.X) //向右移动
            {
                rm.Invoke(dc, new(0, 0, horizonDelta, height));
                if (next.Y < previous.Y)//向上
                    rm.Invoke(dc,
                        new(horizonDelta, height - verticalDelta, width, height));
                else if (next.Y > previous.Y) //向下
                    rm.Invoke(dc, new(horizonDelta, 0, width, verticalDelta));
            }
            else
            {
                if (next.Y < previous.Y) //向上
                    rm.Invoke(dc, new(0, height - verticalDelta, width, height));
                else if (next.Y > previous.Y)
                    rm.Invoke(dc, new(0, 0, width, verticalDelta));
            }
        }
    }
}
