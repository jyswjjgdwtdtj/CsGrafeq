...
namespace CsGrafeq.Shapes
{
    public static class IntersectionMath
    {
            ...
            where TShape2 : GeometryShape
        {
            ...
            if (ss[0] is Line l1)
                if (ss[1] is Line l2)
                {
                    ...
                    return 1;
                }else if (ss[1] is Circle circle1)
                {
                    vs = FromLineAndCircle(l1.Current, circle1.InnerCircle);
                    return 1;
                }else
                    return 0;
            else if(ss[0] is Circle circle1 && ss[1] is Circle circle2)
            {
        ...
        public static (Vec v1,Vec v2) FromTwoCircle(CircleStruct shape1,CircleStruct shape2)
        {
            ...
            if (dis2 > System.Math.Pow(r1 + r2, 2) || dis2 < System.Math.Pow(r1 - r2, 2))
                return (Vec.Invalid,Vec.Invalid);
            var t = Atan2(dy, dx);
            var a = Acos((r1 * r1 - r2 * r2 + dis2) / (2 * r1 * Sqrt(dis2)));
            Vec v3 = new Vec(c1.X + r1 * Cos(t + a), c1.Y + r1 * Sin(t + a));
            Vec v4 = new Vec(c1.X + r1 * Cos(t - a), c1.Y + r1 * Sin(t - a));
            return (v3, v4);
        }
