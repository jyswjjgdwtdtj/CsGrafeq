...
public class MethodInfoHelper
{
    ...
    public MethodInfo Get(string name)
    {
        return Type.GetMethod(name, BindingFlags.IgnoreCase | BindingFlags.Static | BindingFlags.Public) ??
               throw new Compiler.UnMatchException(Compiler.UnMatchException.UnMatchType.FunctionName, name);
    }
    ...
    public MethodInfo Get(string name, int argcount)
    {
        ...
        Get(name);
        throw new Compiler.UnMatchException(Compiler.UnMatchException.UnMatchType.FunctionArgumentsCount,
            name + " " + argcount);
    }
}