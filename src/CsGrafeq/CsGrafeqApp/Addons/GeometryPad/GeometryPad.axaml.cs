...
{
    private AvaPoint PointerPressedPos=new AvaPoint();
    private AvaPoint PointerReleasedPos=new AvaPoint();
    private AvaPoint PointerMovedPos=new AvaPoint();
    private PointerPointProperties PointerProperties=default;
    private List<ActionData> Actions=new List<ActionData>();
    internal ActionData GeoPadAction
    {
        get => field;
        private set
        {
    ...
    public void RadioButtonPointerPressed(object? sender, PointerPressedEventArgs e)
    {
        MessageBox.ShowAtTopLevel(this,"123");
    }
    ...
    protected override bool PointerTapped(AddonPointerEventArgsBase e)
    {
        ...
        else if (GeoPadAction.ShapesToUse.HasFlag(ActionData.UseShape.Point)&&GeoPadAction.Name != "Move" && GeoPadAction.Name != "Select") 
            PutPoint(e.Location).Selected = true;
    ...
    protected override void Render(SKCanvas dc, SKRect rect)
    {
        ...
        {
            var selrect = RegulateRectangle(new Rect(PointerPressedPos,
               ...
               BubbleBack = new() { Color = cd.AxisPaint1.Color.WithAlpha(90), IsAntialias = true })
        {
            ...
            foreach (var p in Shapes.GetPoints())
            {
                ...
                if (string.IsNullOrEmpty(p.Name))
                    ...
                    dc.DrawBubble("Point:" + (char)(Shapes.GetIndex(p) + 65) + " Name:" + p.Name,
