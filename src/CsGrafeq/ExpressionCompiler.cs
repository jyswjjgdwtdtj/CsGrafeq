...
namespace CsGrafeq
{
    internal static class ExpressionCompiler
    {
        ...
        public static CompileResult Compile(string Expression)
        {
            ...
            IntervalImpFunctionDelegate IntervalDg = Compile<IntervalImpFunctionDelegate>( eles, new CompileInfo(typeof(IntervalMath)));
            IntervalSetImpFunctionDelegate IntervalSetDg = Compile<IntervalSetImpFunctionDelegate>( eles, new CompileInfo(typeof(IntervalSetMath)));
        ...
        public static CompileResult Compile(Element[] eles)
        {
            IntervalImpFunctionDelegate IntervalDg = Compile<IntervalImpFunctionDelegate>(eles, new CompileInfo(typeof(IntervalMath)));
            IntervalSetImpFunctionDelegate IntervalSetDg = Compile<IntervalSetImpFunctionDelegate>( eles, new CompileInfo(typeof(IntervalSetMath)));
            MarchingSquaresDelegate msd = GetMSDelegate(eles);
        ...
        private static readonly MethodInfoHelper IntervalMethods = new MethodInfoHelper(typeof(IntervalMath));
        private static readonly MethodInfoHelper  IntervalSetMethods= new MethodInfoHelper(typeof(IntervalSetMath));
        ...
        private static void EmitElements(ILRecorder IL, Element[] eles, MethodInfoHelper mih, char[] param,bool usearray)
        {
            stacklength = 0;
            sb.Clear();
        ...
        private class MethodInfoHelper
        {
            ...
            public MethodInfoHelper(Type type)
            {
                ...
                if (cons == null)
                    throw new Exception("需要有初始化函数");
                ParameterInfo[] parameters = cons.GetParameters();
                if (parameters.Length != 1)
                    throw new Exception("初始化函数需要为单个参数");
                if (parameters[0].ParameterType != typeof(double))
                    throw new Exception("初始化函数参数需要为double类型");
            }
