...
public class NativeBuffer<T> : IDisposable where T : struct
{
    ...
    [DoesNotReturn] private ref T ThrowOutOfRange(nuint index,nuint length) => throw new IndexOutOfRangeException($"Index:{index} Length:{length}");
    ...
    public unsafe NativeBuffer<T> SliceAndDispose(nuint start,nuint length)
    {
