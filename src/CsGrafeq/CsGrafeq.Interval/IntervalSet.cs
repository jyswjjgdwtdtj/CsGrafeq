...
public readonly struct IntervalSet : IInterval<IntervalSet>
{
    ...
    public static readonly IntervalSet Empty = new IntervalSet()
        { Intervals = new Ranges(0), Def = FF, IsNumber = false };
    public static readonly Range EmptyRange = new Range(double.NaN);
    internal Ranges Intervals
    {
        get => field;
        init
        {
    ...
    public static IntervalSet CreateWithNumber(double num) => Create(num);
    ...
    public static IntervalSet Create(double[] nums,Def def)
    {
        ...
        int loc = 0;
        foreach (ref var i in intervals)
        {
            i=new Range(nums[loc++]);
        }
        return new IntervalSet()
        {
            Intervals=intervals,
    ...
    public static IntervalSet Clone(IntervalSet source) => Create(source.Intervals.Clone(), source.Def);
    ...
    private static Range RangeAdd(Range i1, Range i2) => new Range { Inf = i1.Inf + i2.Inf, Sup = i1.Sup + i2.Sup };
    public static IntervalSet operator +(IntervalSet i1, double num)
    {
        foreach(ref var i in i1.Intervals)
        {
            i=new Range(i.Inf+num,i.Sup+num);
        }
        return i1;
    }
    public static IntervalSet operator -(IntervalSet i1, IntervalSet i2)
    ...
    public static IntervalSet operator /(IntervalSet i1, IntervalSet i2)
    {
        ...
        if (i2.IsNumber)
        {
            return i1 * (1 / i2.Number);
        }
        if (i1.IsNumber)
        {
            i1 =IntervalSet.Create(new Ranges(new Range(i1.Number, i1.Number)), i1.Def);
        }
    ...
    public static Def operator !=(IntervalSet i1, IntervalSet i2)
    {
        return ThrowWithMessage<NotImplementedException,Def>(new());
    }
    ...
    public unsafe static IntervalSet operator -(IntervalSet i1)
    {
        i1.Intervals.MonotoneTransformOp(&NumNeg);
        return i1;
    }
    private static double NumNeg(double x) => -x;
    
    #endregion
    #region 数学函数
    public static unsafe IntervalSet Sgn(IntervalSet i)
    {
        if (i.IsEmpty)
        {
            i.Intervals.Dispose();
        ...
        for (nuint j = 0; j< loc; j++)
        {
            res[j]=res[j];
        }
        i.Intervals.Dispose();
        return IntervalSet.Create(res, FT);
    }
    ...
    public static unsafe IntervalSet Median(IntervalSet c1, IntervalSet c2, IntervalSet c3)
    {
        ...
        {
            return Create(DoubleMedian(c1.Number, c2.Number, c3.Number));
        }
        ...
        nuint loc = 0;
        Ranges res=new Ranges(c1.Intervals.Length*c2.Intervals.Length*c3.Intervals.Length);
    ...
    public static unsafe IntervalSet Ln(IntervalSet i1)
    {
    ...
    public static unsafe IntervalSet Lg(IntervalSet i1)
    {
        if (i1.IsNumber)
        {
            i1.Intervals.Dispose();
    ...
    public static IntervalSet Pow(IntervalSet i1, IntervalSet i2)
    {
        ...
        if (i2.IsNumber)
        {
            ...
            if (num == (int)num)
            {
                ...
                if (inum % 2 == 0) //偶数
                {
                    Ranges ranges=new Ranges(i1.Intervals.Length+1);
                    nuint loc = 0;
    ...
    public static unsafe IntervalSet Sqrt(IntervalSet i1)
    {
    ...
    public static unsafe IntervalSet Floor(IntervalSet i1)
    {
    ...
    public static unsafe IntervalSet Ceil(IntervalSet i1)
    {
    ...
    public static unsafe IntervalSet Cot(IntervalSet i1)
    {
    ...
    public static unsafe IntervalSet Cosh(IntervalSet i1)
    {
        ...
        for (nuint i = 0; i < i1.Intervals.Length; i++)
        {
            ref Range r =ref i1.Intervals[i];
            r=r.Contains(0)?
    ...
    public unsafe static IntervalSet ArcCosh(IntervalSet num)
    {
        ...
        return IntervalSet.Create(res, min == min1 ? num.Def : FT);
    }

    public unsafe static IntervalSet ArcSinh(IntervalSet num)
    {
