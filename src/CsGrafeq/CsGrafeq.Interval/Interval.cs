...
using static CsGrafeq.Interval.Interface.IInterval;
using static CsGrafeq.Interval.Interval;
...
public readonly struct Interval : IInterval<Interval>
{
    public Interval(){}
    ...
    {
        CsGrafeq.Math.SwapIfNotLess(ref Inf, ref Sup);
    ...
    public static Interval CreateWithNumber(double num) => Create(num, num, TT);
    ...
    public static Interval operator -(Interval num)
    {
        ...
        return Interval.Create(-num.Sup,-num.Inf,num.Def);
    }
    public static Interval operator -(Interval left, Interval right) => left + (-right);
    public static Interval operator*(Interval left,Interval right)
    {
        if(left.IsInValid ||right.  IsInValid) return _Interval.InValid;
        Def def=left.Def&right.Def;
        if (left.Inf > 0 && right.Inf > 0) return new Interval() { Inf = left.Inf * right.Inf, Sup = left.Sup * right.Sup ,Def=def};
        if (left.Sup < 0 && right.Sup < 0) return new Interval() { Inf = left.Sup * right.Sup, Sup = left.Inf * right.Inf,Def=def };
        var res = Math.GetMinMax4(left.Inf * right.Inf, left.Inf * right.Sup, left.Sup * right.Inf, left.Sup * right.Sup);
    ...
    public static Interval operator/(Interval left,Interval right)
    {
        ...
        if (right.ContainsEqual(0))
            return new Interval() { Inf=double.NegativeInfinity,Sup=double.PositiveInfinity,Def=left.Def&right.Def };
        return Interval.Create(1 / right.Inf, 1 / right.Sup, right.Def) * left;
    }
    public static Interval operator%(Interval a, Interval b)
    {
        if (b.Inf == b.Sup)
        {
            ...
            if (Inf == Sup)
                return Interval.Create(Math.NumMod(b.Inf, num), Math.NumMod(b.Sup, num),a.Def&b.Def);
            return Interval.Create(0, b.Sup,FT);
        }
    ...
    public static Interval Abs(Interval i)
    {
        ...
        {
            return new  Interval() { Inf=0,Sup= sysMath.Max(-i.Inf, i.Sup), Def=i.Def };
        }

        if (i.Sup < 0)
        {
            return new Interval(){Inf=-i.Sup, Sup=-i.Inf,Def=i.Def};
        }
    ...
    {
        return new Interval(){Inf= sysMath.Min(i1.Inf, i2.Inf), Sup=sysMath.Min(i1.Sup, i2.Sup), Def = And(i1.Def, i2.Def)};
    }

    public static Interval Max(Interval i1, Interval i2)
    {
        return new Interval() { Inf = sysMath.Max(i1.Inf, i2.Inf), Sup = sysMath.Max(i1.Sup, i2.Sup), Def = And(i1.Def, i2.Def) };
    }
    ...
    public static Interval Tan(Interval i)
    {
        ...
        if (l - r == 1) return new Interval() {Inf=double.NegativeInfinity,Sup=double.PositiveInfinity, Def = i.Def};
        if (l - r == 0)
        {
            return new Interval(){Inf=sysMath.Tan(i.Inf), Sup=sysMath.Tan(i.Sup), Def = i.Def};
        }
    ...
    public static Interval ArcCosh(Interval i)
    {
        if (i.IsInValid||i.Sup<1)
            return _Interval.InValid;
        Range res = new Range(){Inf=i.Inf, Sup=i.Sup};
        Def def=i.Def;
        if (res.Inf < 1)
        {
            res.Inf = 1;
