...
public static class Compiler
{
    ...
    public static IntervalHandler<T> Compile<T>(string expression) where T : IInterval<T>
    {
        ...
        foreach (Element element in elements)
        {
            switch (element.Type)
            {
                ...
                {
                    if(!double.TryParse(element.NameOrValue, out double d))
                        throw new UnMatchException(UnMatchException.UnMatchType.Number,element.NameOrValue);
                    expStack.Push(Expression.Call(GetInfo(T.CreateWithNumber), Expression.Constant(d)));
                }
                    break;
                ...
                case ElementType.Variable:
                {
                    string name=element.NameOrValue.ToLower();
                        if (name == "e")
                            expStack.Push(Expression.Call(GetInfo(T.CreateWithNumber), Expression.Constant(sysMath.E)));
                        else if (name == "pi")
                            expStack.Push(Expression.Call(GetInfo(T.CreateWithNumber), Expression.Constant(sysMath.PI)));
                        else if (name == "x")
                ...
                {
                    var mf = typeof(T).GetMethod(element.NameOrValue,BindingFlags.Public|BindingFlags.Static|BindingFlags.IgnoreCase);
                    if (mf==null||mf.GetParameters().Length != element.ArgCount)
    ...
    private static MethodInfo GetInfo(Delegate action) => action.Method;
